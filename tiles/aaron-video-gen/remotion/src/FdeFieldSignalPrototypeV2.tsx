...
export const FDE_PROTOTYPE_V2_FPS = 30;
export const FDE_PROTOTYPE_V2_SPEECH_RATE = 1.04;

const narrationOffsetSec = 2.5;
const baseDurationSec = 84.04;

const retimeAbsolute = (time: number): number =>
  time <= narrationOffsetSec
    ? time
    : narrationOffsetSec +
      (time - narrationOffsetSec) / FDE_PROTOTYPE_V2_SPEECH_RATE;

export const FDE_PROTOTYPE_V2_DURATION_SEC = retimeAbsolute(baseDurationSec);

export const fdePrototypeV2DurationFrames = (
  fps = FDE_PROTOTYPE_V2_FPS,
): number => Math.ceil(FDE_PROTOTYPE_V2_DURATION_SEC * fps);

const scenes = {
  prelude: { start: 0, end: narrationOffsetSec },
  convergence: {
    start: narrationOffsetSec,
    end: retimeAbsolute(21.22),
  },
  deployment: {
    start: retimeAbsolute(21.22),
    end: retimeAbsolute(37.66),
  },
  comparison: {
    start: retimeAbsolute(37.66),
    end: retimeAbsolute(58.19),
  },
  actor: {
    start: retimeAbsolute(58.19),
    end: FDE_PROTOTYPE_V2_DURATION_SEC,
  },
} as const;
...
const sceneLocal = (time: number, start: number): number =>
  (time - start) * FDE_PROTOTYPE_V2_SPEECH_RATE;
...
const musicVolume = (frame: number): number => {
  const time = frame / FDE_PROTOTYPE_V2_FPS;
  ...
  if (time < FDE_PROTOTYPE_V2_DURATION_SEC - 3.5) return 0.15;
  return interpolate(
    time,
    [FDE_PROTOTYPE_V2_DURATION_SEC - 3.5, FDE_PROTOTYPE_V2_DURATION_SEC],
    [0.15, 0],
    {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
...
const FrameChrome: React.FC<{ time: number }> = ({ time }) => {
  const route = progress(time, 0, FDE_PROTOTYPE_V2_DURATION_SEC, easeInOut);
  ...
  return (
    <>
      ...
      >
        ...
        <span>
          {String(
            Math.floor(Math.min(time, FDE_PROTOTYPE_V2_DURATION_SEC)),
          ).padStart(2, "0")}
        </span>
      </div>
    </>
      ...
      >
        ...
        <div
          style={{
            ...
            borderRadius: editorial.radius,
            transformOrigin: "left center",
            transform: `translateX(${-turn * 76}px) rotateY(${-58 * turn}deg) translateZ(${turn * 42}px)`,
...
const ActorScene: React.FC<{ time: number }> = ({ time }) => {
  ...
  const recursive = progress(local, 15.05, 17.45, easeInOut);
  const arrowHead = dependentReveal(recursive, 0.985);
  const resolution = progress(local, 19.65, 20.55);
...
const retimedWordTimings = fdePrototypeWordTimings.map((timing) => ({
  ...timing,
...
export const FdeFieldSignalPrototypeV2: React.FC = () => {
  ...
  const endFade =
    ...
    progress(
      time,
      FDE_PROTOTYPE_V2_DURATION_SEC - 0.7,
      FDE_PROTOTYPE_V2_DURATION_SEC,
    );
  ...
  return (
    ...
    >
      <Audio
        src={staticFile("fde-field-signal-prototype/music-edit-v2.mp3")}
        volume={musicVolume}
      />
      <Sequence from={Math.round(narrationOffsetSec * fps)}>
        <Audio
          src={staticFile("fde-field-signal-prototype/narration-v2.mp3")}
        />
      </Sequence>
