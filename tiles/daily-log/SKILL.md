...
Per the repo agent guide ([AGENTS.md](../../AGENTS.md) / [CLAUDE.md](../../CLAUDE.md)) anti-sycophancy principle (cf. Thiago), the skill MUST NOT write the Reflection section. Value of journaling is in self-examination — don't take it away from Aaron.

## When to invoke

- Manual: "daily log", "今日日志", "write today's journal", "log today"
- Headless: Launched by `~/Library/LaunchAgents/com.aaron.daily-log.plist` nightly at 22:00 via `scripts/daily-log.sh`, using the configured agent CLI (`codex` by default, `claude` fallback).
...
5. **Privacy.** Same rules as the repo agent guide — no external APIs, no web search, local-only.
