...
### Five Minutes Can Produce A Draft, Not A System

If someone tells you a few prompts and five minutes produced a great article, they are hiding the production system that made the result possible.

Five minutes may be enough for a model to produce fluent prose. Connecting research, argument, translation, images, audio, video, and publishing is only the first step. Making that entire chain run at a high standard, then improving the standard after every failure, still demands sustained human participation.

My blog workflow is a small example. Version one connected those production steps and made output much faster. It also showed how easily a polished package could rest on weak evidence boundaries, literal translation, repetitive visual language, broken routes, or media that no longer matched the article.

Version two did not emerge from a better master prompt. I had to stay in the loop: reread the drafts, reject weak sections, challenge claims, compare visual directions, test links, and decide which local correction deserved to become a permanent workflow rule. Those decisions produced the claim ledger, argument memo, canon reflection, red-team review, separate English and Chinese editing, concept-first image process, and package validation.

At least today, AI can execute a growing share of the pipeline, but it cannot reliably own the quality bar. It does not consistently recognize what is missing, resolve every editorial tradeoff, or decide which failure should change the system. Without that participation, a recursive loop becomes repeated generation rather than self-improvement.

The goal is not to keep a human manually touching every step forever. The goal is to turn repeated judgment into evidence, standards, evaluations, and reusable rules so the next run starts from a higher baseline.

This personal system does not prove that FDE will work at enterprise scale. It shows why the work requires engineers close enough to reality to notice failures, exercise judgment, and encode what they learn into the deployment and the product. High participation is what makes the feedback loop possible; compounding that participation is what eventually makes the system scalable.
...
## Why Model Companies Are Sending Engineers Into The Field
...
Treating FDE as repackaged consulting is too narrow.
...
That is enough to sharpen the distinction. If an engagement leaves only custom code and dependence on the vendor, it is custom services regardless of the job title. FDE becomes more interesting when the customer can operate what was built and field learning improves the product or the next deployment. ACTOR, introduced below, provides one framework for inspecting whether that loop is real.
...
What remains to be proven is whether FDE can move enterprise AI beyond repeated pilots and into production systems that operate reliably, create value, and improve over time.
