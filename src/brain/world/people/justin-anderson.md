# Justin Anderson — CTO at Mawer

**Role:** CTO at [[orgs/mawer]]
**Relationship to Aaron:** Former closest ally, now complicated 🟡
**Trust level:** Medium (personal yes, strategic diverging)
**Distance:** Regular work interaction
**Last updated:** 2026-04-10

## Position
- Reports to [[people/keri]] (COO)
- Background: financial analyst, **not a programmer**
- Initiated [[projects/nova]] with [[people/wilson]]
- On vacation ~mid-March to early April 2026 (3 weeks, right before decision deadline)

## Who He Is
- Big vision, weak execution follow-through
- History of big projects abandoned after 6-12 months
- Became AI-obsessed after the 2024-25 hype wave
- Posts at 5-6 AM, v0 mockups constantly
- Sees Mawer as "dead business" — cultural mismatch with reality
- Tends to "tear down and rebuild" vs Aaron's "augment existing"

## Relationship to Aaron
- **提拔之恩** — Justin promoted Aaron earlier in his career, real debt of gratitude
- Aaron's loyalty is to **"去年的 Justin"** not the current one
- Was Aaron's closest technical ally last year
- Displaced Aaron as most trusted since Wilson became the AI partner
- Still personally warm, but strategic distance growing

## Pattern: April 2026 Behavior
- **Post-COO retreat (April 9):** Sent "proud/honored/world-changing" message with zero accountability for demo failures. Full PR mode.
- **Assigned Aaron to lead Campaign UX polish** for 7-day sprint before April 17 deadline
- **Asked Aaron for in-person meeting** on April 10 — signals wanting closer ally re-engagement
- **Missed team meeting** on April 9 (COO retreat day 2)
- Defined "Interaction" in vibe-coding language — marketing disguised as tech spec

## The Nova Narrative
- Claims Nova "goes beyond agentic orchestration"
- Claims Nova flows have "greater ability than Claude Skills"
- "Interaction" defined as "purely vibe-coded" — writes unstable methodology into product DNA
- Refuses to acknowledge retreat failures as failures

## How Aaron Engages
- Professional, engaged, technically focused
- **Not sycophantic** — Justin respects people with backbone
- Support without backing the narrative
- Document scope tightly to avoid scapegoat risk
- Don't reference Keri's shared info
- Never bash Wilson or Alex to him
- Don't doubt Nova aloud
- Don't mention OrgNext

## Strategic Reading
- ~50% probability: Justin and/or Wilson pushed out or leave → Aaron inherits business
- ~50%: Justin survives, Nova limps along, Aaron plays UX role without owning blame
- Either way: Aaron's strategy is same — deliver real value, keep scope tight, don't bet career on Nova

## Communication Pattern Analysis (April 15)
- Uses "he/him" when talking about Aaron and Chintan in group channels (they're present) — signals hierarchy, treating them as assigned executors vs peers
- Contrasts with "Duncan we can call in...", "Wilson we can call in..." — direct address, peer tone for inner circle
- **Inner circle = Builders (Wilson, Duncan, Narek). Outer circle = Orchestrators (Aaron, Chintan).** Language reflects this unconsciously.
- Narcissistic leadership traits: epic narrative language ("written in blood", "Release the Kraken"), creates urgency that suppresses rational questioning, selective recognition (Alex gets public credit for narrative, Aaron's io_spec/UX work gets "he can showcase")
- PUA-adjacent pattern: 提拔之恩 as psychological leverage → gradual displacement without explicit communication →归属感焦虑 through exclusion language → vision used to pressure compliance

## Narrative vs Reality Pattern (April 15)
- Research adoption is real — Joshua Samuel using M42 MCP + Xpress Feed at 2:56 AM, genuine usage
- **But Research is using Claude + MCP, not Nova's Quest/Campaign system.** The actual value comes from Anthropic's protocol + data feeds
- Justin packages Research's independent AI exploration as "Nova's success" — classic corporate move: brand other people's work under your platform
- Research doesn't care about the label — they get visibility and model access. Justin gets political capital. Mutual exchange.
- Peter Lampert + Vijay Viswanathan now willing to advocate to ET/Board — Justin successfully created evangelists without Nova platform itself being stable
- **Nova's survival is political, not product-driven.** This explains why it keeps surviving despite repeated demo failures.

## Information Discipline (Lesson learned April 17)
- Aaron shared PW numbers ($13B→$26B) and David's ambition with Justin — too much detail
- Justin now knows Aaron's strategic direction and may try to link Nova to PW
- **Rule going forward:** Direction-level info only with Justin ("excited about client side opportunities"). No numbers, no names, no specifics. Strategic details only with Keri/David/Thiago.

## Nova Exit Negotiation (April 17)
- Aaron negotiated reduced Nova involvement: Campaign UX polish only, spare cycles, no daily meetings, sync with Duncan only
- Justin tried to keep Aaron at "1/2 allocation" — Aaron pushed back to spare time
- Justin's response: "up to you... I'd recommend staying tied in (if I were you)" — soft retention pull, not a command
- Aaron confirmed: stay in chat group, no meetings, polish campaign UX by May 25
- Also committed to MAP completion (SD team adoption) and seg trading before full PW focus

## April 17 Extended Chat — Key Signals
- **Threw Wilson under the bus:** "Wilson has his weaknesses... Wilson cannot help on that side with his comms" — rare, unprompted. Testing Aaron's allegiance? Or genuine frustration?
- **Acknowledged Aaron's underperformance framing:** "I still feel you contribute far below your potential (maybe in part because your headspace was focusing on what is next)" — true observation but also building a narrative he can use later if needed ("Aaron's heart wasn't in it")
- **Rare self-reflection:** "I was too slow to see tbh" (re: Client Lab opportunity) — unusual for Justin. Store this.
- **Admitted needing Aaron for business advocacy:** "I have to carry the business side advocacy almost alone" — Wilson can't do comms, Duncan is pure builder. Justin actually needs a business translator and sees Aaron as that person.
- **Still trying to keep Aaron close:** "you could become a huge power user of Nova" / "helping me with the transition" — soft pull continues even after exit was agreed
- **Aaron's closing was good:** "see me as a partner still" + "let me crack client side, let it be the engine for Mawer" — maintained relationship without over-committing

## Observations Timeline
- 2026-04-10: In-person meeting scheduled (UX discussion + likely political processing)
- 2026-04-09: Post-retreat public PR message to Nova builders — zero accountability, full "world-changing" language, assigned Aaron to Campaign UX
  ref: [[_archive/2026-04-09/justin-post-retreat-message]]
- 2026-04-09: Direct message to Aaron narrowing priority — "io_spec setting process" is the real #1
  ref: [[_archive/2026-04-09/justin-direct-message-aaron-ux]]
- 2026-04-09: Shared definition of "Interaction" featuring "purely vibe coding" and "greater than Claude Skills" claims — marketing language disguised as tech spec
  ref: [[_archive/2026-04-09/justin-interaction-definition]]
- 2026-04-08: COO retreat demo failed (Redis overload, vendor app incomplete)
- 2026-03-19 (approx): Mar 17 start of 3-week vacation, right before March 31 then April 17 deadline
- 2026-03 earlier: Lost closest-ally status to Wilson in Justin's inner circle

## What to Watch
- Is he becoming more or less connected to reality about Nova's state?
- Does he start asking Aaron for honest reads vs. narrative support?
- Vacation behavior — working through it or disengaging?
- Does his "retreat in May" for Nova team actually happen, or slip?
- ET-level confidence in him — any signals from [[people/keri]]?

## Open Questions
- Does he have a personal exit strategy (startup? senior role elsewhere?)
- How much of his behavior is conviction vs. sunk-cost momentum?
- Would he recognize if Nova failed, or would he rebrand and continue?

---

**Category:** Boss with history; strategically diverging
**Political weight:** High — his rise or fall affects Aaron's immediate environment significantly
**Red lines:** Never bash him to Keri; never leverage 提拔之恩 transactionally
