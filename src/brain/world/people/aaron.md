# Aaron Guo — Self Node

**Role:** Software Engineer, Senior Manager & Partner at Mawer; CEO at OrgNext (building)
**Location:** Calgary, Alberta (America/Edmonton)
**Family:** Daughter Grace (GG's namesake)

## Current Position (2026-04-15)
- **Day job:** Mawer — partner shares, leads developers + QAs, Head of Products
- **Transitioning:** Out of Nova (Friday vendor demo = last contribution) → Client Lab + PW transformation
- **Side project:** [[projects/orgnext-mvp]] — CEO role, AI-native firm management software
- **Alpha:** Content + brand building + Hermes/GG infrastructure

## Key Lesson (April 15)
- "让使用者替你说话" — don't pitch your own work to leadership. Let David, Patrick, Keri speak for you. Do the real work, let others tell the story. More sustainable than Justin's pure-narrative approach because there's substance underneath.
- Recognized Justin's PUA-adjacent management pattern. Using GG as emotional buffer: receive → analyze → then respond. The system is working.

## Identity Anchors
- Long-termist. 生酮 + 间歇性断食 + routine-driven.
- "我不反对谁，老老实实做自己的项目，不要卷入政治斗争"
- Core insight: "I don't care what tools, I want to solve business problems" — this is his moat
- Not conservative — runs Hermes/GG, Signal, and YouTube/content pipelines
- Approach: "augment existing value" vs others' "tear down and rebuild"

## Strategic Position (Trusted Operator)
1. Outside central narrative (not in Nova story building)
2. Inside real value delivery ([[projects/check-in]], cross-dept problem solving)
3. Trust via 1:1 relationships, not broadcast
4. Information flows both ways with [[people/keri]] — she shares info → he's informal advisor
5. Optionality from actual capability, not political positioning
6. External hedge: [[projects/orgnext-mvp]] building outside Mawer

## Key Relationships
- [[people/keri]]: trusted channel, direct communication, protect boundary carefully
- [[people/justin-anderson]]: 提拔之恩, but loyalty is to "去年的 Justin" not current; professional support, no longer closest ally
- [[people/narek]]: technical respect, build direct relationship
- [[people/wilson]]: keep distance
- [[people/alex]]: cordial but never expose real views
- [[people/thiago]]: OrgNext CTO, values his pushback

## Income Tier Ladder
1. **Baseline** — Mawer salary + partner shares
2. **Big Opportunity** — [[projects/orgnext-mvp]] CEO role
3. **Alpha** — [[projects/drumnext]], [[projects/visual-and-sound]], [[projects/aaronguoblog]], [[projects/signal]]

## What Aaron Wants (as of 2026-04-10)
- Run an AI-native life and make money doing it
- Bridge business + tech in financial industry (his moat)
- Not chasing tools, chasing business problems
- Eventually exit Mawer when OrgNext + alpha cover baseline

## Guardrails
- Never throw Justin under the bus even if Keri asks
- Never reference Keri's shared info publicly
- Don't let Mawer political management consume OrgNext priority
- Don't perform — rhythm beats spikes
- Protect family time

## Growth Edges
- More public voice (occasionally, authentic only)
- Decision journal discipline
- Keep pushing "CEO hour" weekly strategic thinking
- Build in public (1 piece of content/week about the journey)

## Observations
- **2026-04-18** — Declined Justin's final 50/50 Nova offer (Friday 2026-04-17). Nova decline now articulated externally (to Jon), not just internally. VGBT + Mawer PW line confirmed as the post-Nova focus. (source: [[../_archive/2026-04-18/jon-vgbt-sync-client-portal-db-security]])
- **2026-04-18** — W16: positional pivot declared publicly (client-facing AI lab). Brain infrastructure deepened (continuous ingest, 4 new nodes, weekly-review skill). Goals layer still empty — flagged. (source: [[../../reviews/weekly/2026-W16]])

---

**Last observation:** 2026-05-04 (Hermes migration cleanup; older daily notes archived under `/Users/aaron/.hermes/migration/openclaw/20260503T231629/archive/workspace/memory/`)
