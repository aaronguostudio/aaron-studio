...
![Anthropic金融AI技术栈蓝图架构图——顶层为Claude + Skills，中层为四个专业模块，底层为11个机构级数据连接器。Apache 2.0开源，2026年2月发布。](imgs/00-cover.jpg)
...
![Repo架构：financial-analysis核心插件、四个专业插件、11个MCP数据连接器](imgs/01-framework-repo-architecture.jpg)
...
![MCP协议汇聚：11家机构级数据供应商全部接入MCP协议，经由Claude输出结果](imgs/02-infographic-mcp-convergence.jpg)
...
![AMZN DCF模型 Excel 内页——情景假设、收入构建、WACC与敏感性分析表](imgs/02-dcf-output-excel.png)

**客户报告的产出：**

报告输出（使用样本数据生成，非真实客户信息）同样结构清晰——执行摘要、关键指标、组合评述，格式针对客户场景。语言专业，结构标准。同样不是直接发出去的状态，但是扎实的初稿，把"空白页问题"完全消除了。

![客户报告产出——面向客户的格式化报告](imgs/03-client-report-output.png)
...
![面向技能的编程——专业知识编码在结构化文件中，而非代码里](imgs/07-skills-oriented-programming.jpg)
...
我利用财富管理技能，用一个样本客户档案跑了一遍——产出不言自明。

![产出：基于技能文件生成的个性化投资建议，按公司标准格式化](imgs/06-investment-recommendation-output.png)
...
![范式转变：搭建 Agentic 工作流（数周工程量）vs 写一个技能文件（数小时）](imgs/03-comparison-agentic-vs-skill.jpg)
...
![快速落地 vs 需要更长时间——中型金融机构的采用现实框架](imgs/04-comparison-adoption-framework.jpg)
...
![蓝图——AI原生金融机构架构：顶层为技能文件中的机构知识，中层为核心基础设施与专业模块，底层为数据连接器，产出至Excel/PowerPoint/Word](imgs/08-blueprint-ai-native-firm.jpg)
