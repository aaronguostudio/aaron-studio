...
![小明站在怀石料理台前，背后是整面墙的 4,192 个判断旋钮](illustrations/01-scene-ming-hero.png)
...
![32,000 套怀石记录叠成一摞卷轴，其中一卷展开显示序列 e→m→m→a，旁边是 ChatGPT 的万亿条序列对比气泡](illustrations/02-infographic-training-data.png)
...
![分词流水线：「emma」→ 带服务铃的 token → 数字 26,4,12,12,0,26 → 附带 16 个格子的风味档案卡](illustrations/03-infographic-tokenization-pipeline.png)
...
![三站式出菜流水线：副厨圆桌 → 后厨加工 → 最终投票，输出为 27 道菜的概率排名](illustrations/04-flowchart-assembly-line.png)
...
![俯视视角的副厨圆桌：四位副厨（甲=浓郁度，乙=口感，丙=温度，丁=酸度）围坐，中央盘子上写着「第四道菜：m」](illustrations/05-scene-sous-chef-roundtable.png)
...
![后厨工作法三步：全面打开（发散的箭头） → 品控筛选（划掉的路径） → 收拢判断（汇聚成单一箭头）](illustrations/06-flowchart-back-kitchen-method.png)
...
![两个幕后配角：漱口（每个站点前重置到中性基准）和混入一勺原味（每个站点后把原始档案混回去）](illustrations/07-infographic-supporting-roles.png)
...
![食客打分卡：三种把握程度对应的惩罚，以及损失值从 3.3 降至 2.37 的成长曲线](illustrations/08-infographic-loss-scorecard.png)
...
![反向传播链：「太咸了」← 摆盘 ← 后厨加工 ← 副厨圆桌 ← 旋钮 #347「酱油浓度调节」](illustrations/09-flowchart-backpropagation-chain.png)
...
![六种烹饪手法 2×3 图格：合并、融合、浓缩、提取、发酵、品控，每个配手绘烹饪图标](illustrations/10-infographic-six-techniques.png)
...
![训练循环：出菜→打分→追溯→调整（1,000 轮），旁边是师傅的三种方法：记忆、因材施教、越来越轻](illustrations/11-framework-training-loop-adam.png)
...
![冒险旋钮温度表：0.1（极度保守）到 0.7（ChatGPT 标星）到 1.5（大胆冒进），三档各配对应风格的出菜示例](illustrations/12-comparison-temperature-dial.png)
...
![规模对比：小明的街边小摊（4,192 个旋钮、MacBook、32,000 套记录）vs ChatGPT 的米其林厨房（数千亿旋钮、数千块 GPU、万亿条记录）。页脚：「同样的烹饪原理，天壤之别的规模。」](illustrations/13-comparison-scale-ming-vs-chatgpt.png)
